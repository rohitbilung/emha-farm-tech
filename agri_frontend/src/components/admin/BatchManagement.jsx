...
import RegisterBatchModal from '../../forms/RegisterBatchModal';
...
const BatchManagement = () => {
  ...
  const [farmers, setFarmers] = useState([]); // List for the dropdown
  ...
  useEffect(() => {
    const fetchFarmers = async () => {
      try {
        const res = await api.get(API_ENDPOINTS.GET_FARMERS, {
          params: { role: "farmer" }
    ...
    try {
      const response = await api.get(API_ENDPOINTS.GET_BATCHES, {
        params: {
          ...
          limit: 10,
          search: searchTerm,
          farmerId: farmerFilter // Send the farmer ID to the backend
        }
      });
  ...
  }, [currentPage, searchTerm, farmerFilter]); // Add farmerFilter to dependencies
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-green-600 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-green-700 transition shadow-lg shadow-green-900/10">
          <Layers size={18} /> Register New Batch
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <div className="relative min-w-[200px]">
          ...
          <select
            ...
            className="w-full pl-12 pr-10 py-3 bg-white border border-stone-200 rounded-2xl outline-none focus:border-green-500 transition font-bold text-stone-600 appearance-none cursor-pointer"
          >
